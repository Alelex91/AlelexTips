...
const MatchCard: React.FC<Props> = ({ prediction, onPlay }) => {
  ...
  const getDefaultImage = (sport: string) => {
    ...
    if (s.includes('rugb'))
      return 'https://images.unsplash.com/photo-1544698310-74ea9d1c8258?q=80&w=800';
    if (s.includes('f1') || s.includes('formu') || s.includes('racing'))
      return 'https://images.unsplash.com/photo-1509064322244-f6983a4af701?q=80&w=800';
    if (s.includes('moto'))
      return 'https://images.unsplash.com/photo-1558981403-c5f9899a28bc?q=80&w=800';
    ...
    <div className={`glass-morphism rounded-[2.5rem] border-white/5 hover:border-[#00FF66]/40 transition-all duration-500 overflow-hidden relative group h-full flex flex-col ${isExpanded ? 'ring-2 ring-[#00FF66]/30' : ''}`}>
      ...
      <div className="p-6 sm:p-7 flex flex-col flex-1">
        ...
        <div className="mt-auto space-y-4">
            <button onClick={() => setIsExpanded(!isExpanded)} className="w-full py-4 bg-[#0a0c0e] rounded-2xl text-[9px] font-black uppercase text-slate-400 border border-white/5 hover:text-[#00FF66] hover:border-[#00FF66]/20 transition-all flex items-center justify-center gap-2 tracking-[0.2em]">
                ...
                <div className="animate-slideUp p-5 bg-emerald-500/5 rounded-3xl border-l-4 border-[#00FF66] space-y-4 mb-4">
                    ...
                    <div className="flex gap-3">
                        <div className="flex-1 bg-black/40 p-3 rounded-2xl text-center border border-white/5">
                            <p className="text-[6px] text-slate-500 uppercase font-black tracking-widest mb-1">xG/Stat Flow</p>
            ...
            <button onClick={() => onPlay?.(prediction)} className="w-full py-5 bg-[#00FF66] text-black rounded-[2rem] font-black uppercase text-[11px] tracking-[0.3em] shadow-[0_0_20px_rgba(0,255,102,0.2)] hover:shadow-[0_0_35px_rgba(0,255,102,0.4)] transition-all active:scale-95">
                Add To Node
            </button>
        </div>
      </div>
